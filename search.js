...
function getChart() { // MAINTAINENCE
      ...
      for(r = 0; r <= 30; r++ ) {
        if(players[player][togglesToArray[i]] !== undefined) {
          if(players[player][togglesToArray[i]][r] !== undefined) {
            ...
            }else if(subCheckDecider.checked) { //if sub
              if(players[player][togglesToArray[i]][r][games[0]] !== undefined) {
                valueSB = 0;
                countSB++;
                valueSB = players[player][togglesToArray[i]][r][games[0]];
  ...
  pkwBar.update();
  pie.data.datasets[0].data[0] = (coinsSB / countSB) * 1.5;
  pie.data.datasets[0].data[1] = coinsTGTTOS / countTGTTOS;
  pie.data.datasets[0].data[2] = coinsHITW / countHITW;
  pie.data.datasets[0].data[3] = coinsBB / countBB;
